...
export async function getCSAMemberships(): Promise<csaMembership[]>{
    ...
    const memberships: csaMembership[] = await directus.request(readItems("csa_membership"));
    return memberships;
} 

export async function getCSAMembershipsOfCurrentUser(): Promise<csaMembership[]>{
    const directus = await useDirectus();
    const user = await directus.request(readMe({ fields: ["*"] }));
    const memberships: csaMembership[] = await directus.request(readItems("csa_membership", {filter: {csa_membership_of: user.id}}));
    return memberships;
}
...
export async function addNewMembership(id: string ){
    const directus = await useDirectus();
    const newMembership = await directus.request(createItem("csa_membership", { csa_membership_of: id}));
...
export async function deleteCSAMembership(id: number){
    const directus = await useDirectus();
    const deletedMembership = await directus.request(deleteItem("csa_membership", id));
    return deletedMembership;
}

export async function getFieldsByCollection(collection_name: string) {
    const directus = useDirectus();
    const result = await directus.request(readFieldsByCollection(collection_name));
    return result;
}
